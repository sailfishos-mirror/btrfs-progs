...
static int remove_data_reloc_tree(struct btrfs_trans_handle *trans)
{
	 ...
	 * double-check that we're not going to mess things up because the tree
	 * has multiple leaves.
	 */
...
static int create_remap_tree(struct btrfs_trans_handle *trans)
{
	...
	ret = btrfs_alloc_chunk(trans, fs_info, &chunk_start, &chunk_size, bg_flags);
	if (ret)
		return ret;

	ret = btrfs_make_block_group(trans, fs_info, 0, bg_flags, chunk_start, chunk_size);
	if (ret)
		return ret;
...
int convert_to_remap_tree(struct btrfs_fs_info *fs_info)
{
	...
	if (!ret) {
		btrfs_release_path(&path);
		error("cannot convert filesystem that has balance already in progress");
		return -EINVAL;
	}
